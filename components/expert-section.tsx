  ...
  return (
    <section id="experts" ref={sectionRef} className="py-24 bg-gray-50">
      <div className="container mx-auto px-6">
        <div
          className={`text-center mb-20 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} transition-all duration-1000`}
        >
          <h2 className="text-5xl font-bold text-gray-900 mb-6 text-balance">Learn From The Experts</h2>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Get personalized insights from industry leaders who've shaped the finance world
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 max-w-5xl mx-auto">
          {experts.map((expert, index) => (
            <div
              key={index}
              className={`group bg-white rounded-3xl p-10 shadow-xl hover:shadow-2xl transition-all duration-500 border border-gray-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                }`}
              ...
              <div className="text-center mb-8">
                <div className="w-32 h-32 mx-auto rounded-full overflow-hidden shadow-xl mb-6 ring-4 ring-white">
                  <Avatar className="w-full h-full">
                    <AvatarImage src={expert.avatar || "/placeholder.svg"} alt={expert.name} className="object-cover" />
                ...
                <p className="text-lg text-gray-600 mb-1 font-medium">{expert.designation}</p>
                <p className="text-[#294085] font-bold text-xl mb-4">{expert.company}</p>
                ...
                <p className="text-gray-600 leading-relaxed mb-8">{expert.bio}</p>

                <div className="flex flex-wrap gap-2 justify-center mb-8">
                  {expert.expertise.map((skill, i) => (
                    <span key={i} className="bg-blue-50 text-[#294085] px-4 py-2 rounded-full text-sm font-medium">
                      {skill}
                    </span>
                  ))}
                </div>
